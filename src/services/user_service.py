...
class UserService:
    ...
    async def create_new_user(self, new_user: UserCreate) -> UserCreate:
    ...
    async def delete_user(self, credentials: HTTPAuthorizationCredentials) -> dict:
        user_to_delete: UserModel = await self.auth_service.get_current_token_payload(credentials=credentials)
    ...
    async def update_user(self, user_update: UserUpdate, credentials: HTTPAuthorizationCredentials) -> UserModel:
        current_user: UserModel = await self.auth_service.get_current_token_payload(credentials=credentials)
    ...
    async def read_current_user(self, credentials: HTTPAuthorizationCredentials):
        return await self.auth_service.get_current_token_payload(credentials=credentials)
